              ...
              <div class="field-grid cols-2" ${!settings.confidentiality_acknowledged ? 'style="opacity:0.4;pointer-events:none;"' : ''}>
                ...
                ${isAzure ? `
                  ...
                  <label class="field">${tt('API version', 'Version de l\'API', 'API-Version')}
                    ...
                    <p class="helper">${tt('"preview" (default) uses Azure\'s unified v1 API and works with any deployment. If your resource only supports the legacy API, enter a dated version instead (e.g. 2024-10-21) — the request will then be sent to the classic per-deployment endpoint.', '« preview » (par défaut) utilise l\'API v1 unifiée d\'Azure et fonctionne avec tout déploiement. Si votre ressource ne prend en charge que l\'ancienne API, saisissez une version datée (ex. 2024-10-21) — la requête sera alors envoyée vers l\'ancien endpoint par déploiement.', '„preview" (Standard) verwendet Azures einheitliche v1-API und funktioniert mit jeder Bereitstellung. Falls Ihre Ressource nur die alte API unterstützt, geben Sie stattdessen eine datierte Version ein (z. B. 2024-10-21) — die Anfrage wird dann an den klassischen Endpunkt pro Bereitstellung gesendet.')}</p>
                  </label>
