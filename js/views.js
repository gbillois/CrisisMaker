              ...
              <div class="field-grid cols-2" ${!settings.confidentiality_acknowledged ? 'style="opacity:0.4;pointer-events:none;"' : ''}>
                ...
                ${isAzure ? `
                  ...
                  <label class="field">${tt('API version', 'Version de l\'API', 'API-Version')}
                    ...
                    <p class="helper">${tt('A dated version (default, e.g. 2024-10-21) calls the classic per-deployment endpoint, which works with direct browser requests. "preview" or "latest" switches to Azure\'s newer unified v1 API — required by some newly created (Foundry) resources, but it does not accept direct calls from a browser (CORS), so it only works if you route requests through your own server-side proxy.', 'Une version datée (par défaut, ex. 2024-10-21) appelle l\'ancien endpoint par déploiement, compatible avec les appels directs depuis le navigateur. « preview » ou « latest » bascule vers la nouvelle API v1 unifiée d\'Azure — requise par certaines ressources récentes (Foundry) — mais elle n\'accepte pas les appels directs depuis un navigateur (CORS) ; elle ne fonctionne donc que si vous passez par un proxy côté serveur.', 'Eine datierte Version (Standard, z. B. 2024-10-21) ruft den klassischen Endpunkt pro Bereitstellung auf, der direkte Browseranfragen unterstützt. „preview" oder „latest" wechselt zur neueren einheitlichen v1-API von Azure — von manchen neu erstellten (Foundry-)Ressourcen benötigt —, akzeptiert aber keine direkten Anfragen aus dem Browser (CORS) und funktioniert daher nur über einen eigenen serverseitigen Proxy.')}</p>
                  </label>
