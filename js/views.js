      ...
      function renderLLMConfigBlock(zone, placeholder, options = {}) {
        ...
        const generateLabel = loading
          ? (options.loadingLabel || tt('Generating…', 'Génération en cours…', 'Wird generiert…'))
          ...
          <section class="grid cols-2">
            <article class="card">
              ...
              <div class="actions" style="margin-top:18px;">
                <button class="btn btn-primary" data-action="test-connection" ${connectionTest.status === 'testing' ? 'disabled' : ''}>${connectionTest.status === 'testing' ? tt('Testing…', 'Test en cours…', 'Wird getestet…') : tt('Test connection', 'Tester la connexion', 'Verbindung testen')}</button>
      ...
      function renderFieldControl(stimulus, spec) {
        ...
        const genBtn = `<div class="actions" style="margin-top:4px;"><button class="btn btn-ghost" style="font-size:0.82rem; padding:6px 10px;" data-action="generate-field" data-stimulus-id="${stimulus.id}" data-field-name="${spec.key}">✨ ${tt('Regenerate', 'Régénérer', 'Neu generieren')}</button></div>`;
